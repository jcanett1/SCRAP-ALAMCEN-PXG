...
const TABLE_NAME = "scrap_pxg_componentes_proceso";

export default function ScrapProceso() {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const { data: records = [], isLoading: isLoadingRecords, refetch } =
    trpc.scrap.proceso.list.useQuery({ limit: 20 });

  const insertMutation = trpc.scrap.proceso.insert.useMutation({
    onSuccess: () => {
